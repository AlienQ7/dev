<?php
// index.php (V7.5.5 - FINAL WORKING VERSION)
...
function checkDailyReset(&$user, $dbManager) {
    ...
    if ($user['last_task_refresh'] < $today_midnight_ts) {
        ...
        foreach ($tasks as $task) {
            // FIX #2: Only permanent tasks are kept and reset. Non-permanent tasks are discarded.
            if (($task['permanent'] ?? false) === true) {
                // FIX #3: Reset completed/claimed state for permanent tasks
                $task['completed'] = false;
                $task['claimed'] = false; 
                $updatedTasks[] = $task;
            } 
            // else: non-permanent tasks are automatically dropped from $updatedTasks
        }
        ...
        foreach ($tasks as $key => &$task) {
            if (($task['id'] ?? null) === $taskId) {
                ...
                if ($action === 'toggle') {
                    ...
                    if ($task['completed']) {
                        // Task is now COMPLETED (TICKED)
                        $canClaim = (($task['claimed'] ?? false) === false || ($task['permanent'] ?? false) === true);
                        ...
                        // Task is now UN-COMPLETED (UN-TICKED)
                        
                        // FIX #5: Deduct points on un-ticking for lock/non-lock tasks
                        if ($taskWasCompleted) {
                            $user['claimed_task_points'] -= $reward;
                            ...
                            // Reset claimed status for non-permanent tasks only
                            if (($task['permanent'] ?? false) === false) {
                                $task['claimed'] = false;
                            }
                        }
                ...
                if ($action === 'delete') {
                    // FIX #4: Decrement daily completed count if the deleted task was completed
...
<script>
    ...
    async function togglePermanent(id, newPermanentStatus) {
        ...
        if (result.success) {
            ...
            // Always reset task when toggling permanence
    ...
    async function collectSp() {
        ...
        if (result.success) {
            ...
            updateStatsDisplay({
                ...
                sp: result.sp_points,
                // Check if failed-stats exists before accessing textContent
    ...
    async function saveObjective() {
        ...
        if (result.success) {
            ...
            // Reload if the objective is cleared to show the placeholder cleanly
            if (objective === '') {
                window.location.reload(); 
            }
        } else if (result.message !== 'Maintenance Mode') {
